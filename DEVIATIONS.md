...
After these fixes, all three CI jobs verified green locally against the actual commands each workflow step runs: `pnpm turbo run lint type-check` (passes except the pre-existing `packages/ui` gap, confirmed independently reproducible on plain `main`), `uv run ruff check . && uv run ruff format --check . && uv run pytest` (passes; pytest still collects 0 tests — pre-existing, exits 0), and the full TS backend pipeline (`check:rls` → build 4 packages → type-check 5 packages → `cd services/core-api && pnpm test`, 9/9 passing against real Postgres/Redis).
